...
export default function TravelPackDownload({ pack }: TravelPackDownloadProps) {
  ...
  // --- DESKTOP STANDALONE EXPORT LOGIC ---
  const handleDesktopExport = async () => {
    // Safe extraction of tier data to avoid TS property errors
...
<head>
    ...
    <style>
        ...
        .app-shell { max-width: 450px; margin: 0 auto; min-height: 100vh; background: #020617; border-left: 1px solid rgba(255,255,255,0.05); border-right: 1px solid rgba(255,255,255,0.05); position: relative; box-shadow: 0 0 100px rgba(0,0,0,0.5); }
        .glass-card { background: rgba(30, 41, 59, 0.4); border: 1px solid rgba(255, 255, 255, 0.08); backdrop-filter: blur(16px); border-radius: 32px; }
...
<body>
    <div class="app-shell pb-32">
        ...
        <main id="offline-main" class="px-6 space-y-6">
            <div id="loader" class="text-slate-500 text-[10px] font-black uppercase tracking-[0.3em] text-center py-10">Decrypting Tactical Data...</div>
        </main>
        <footer class="fixed bottom-0 left-0 right-0 max-w-[450px] mx-auto p-6 bg-gradient-to-t from-[#020617] via-[#020617] to-transparent">
            <div class="glass-card py-5 px-8 flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-slate-500">
                <div class="flex flex-col">
                    <span class="text-emerald-500 font-bold tracking-widest">ASSET SECURED</span>
                    <span class="text-[8px] opacity-50">VERIFIED: \${JSON.parse('${cityData}').exportDate}</span>
                </div>
    ...
    <script>
        ...
        if (data.sections && data.sections.length > 0) {
            ...
            data.sections.forEach((section, idx) => {
                ...
                el.className = 'glass-card p-8 animate-up';
                el.style.animationDelay = (idx * 0.1) + 's';
                el.innerHTML = \`
                    <div class="flex items-center gap-3 mb-5">
                        <div class="w-1 h-5 bg-emerald-500 rounded-full shadow-[0_0_10px_#10b981]"></div>
                        <h2 class="text-emerald-400 text-[11px] font-black uppercase tracking-widest font-bold">\${section.title}</h2>
                    </div>
  ...
  const handleMainAction = async () => {
    ...
    if (isMobile) {
      ...
      // Desktop Standalone HTML Export
    ...
    <div className="w-full space-y-3">
      ...
      {status === 'saved' && (
        <p className="text-[11px] text-center text-slate-400 font-bold uppercase tracking-widest animate-fadeIn">
          {isMobile ? "Saved to local vault • No signal required" : "Encrypted Standalone File Exported"}
        </p>
      )}
          ...
          <div className="bg-white text-slate-900 rounded-[40px] p-8 max-w-sm w-full shadow-2xl text-center">
            ...
            <p className="text-slate-600 text-sm mb-8 leading-relaxed">
              {isMobile 
                ...
                : 'Standalone interactive app file has been generated for offline use.'}
            </p>
